...
HIVE PATCHED for python 3.7 to get rid of reserved async
...
class Channel(virtual.Channel):
    ...
    def _connparams(self, is_async=False, _r210_options=(
    ...
    def _create_client(self, is_async=False):
        if is_async:
            return self.AsyncClient(connection_pool=self.async_pool)
        return self.Client(connection_pool=self.pool)

    def _get_pool(self, is_async=False):
        params = self._connparams(is_async=is_async)
    ...
    def async_pool(self):
        if self._async_pool is None:
            self._async_pool = self._get_pool(is_async=True)
        return self._async_pool

    @cached_property
    def client(self):
        """Client used to publish messages, BRPOP etc."""
        return self._create_client(is_async=True)
