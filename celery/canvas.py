    ...
    def election(self):
        ...
        with app.producer_or_acquire(None) as P:
            ...
            app.control.election(
                tid, "task", self.clone(task_id=tid, **props), connection=P.connection
            )
...
class _basemap(Signature):
    ...
    def apply_async(self, args=(), kwargs={}, **opts):
        ...
        return self.type.apply_async((), {"task": task, "it": list(it)}, **opts)
...
class chunks(Signature):
    ...
    def from_dict(self, d, app=None):
        return _upgrade(
            d,
            chunks(*self._unpack_args(d["kwargs"]), app=app, **d["options"]),
        )
...
def _maybe_clone(tasks, app):
    return [
        s.clone() if isinstance(s, Signature) else signature(s, app=app) for s in tasks
    ]
...
class group(Signature):
    ...
    def apply_async(self, args=(), kwargs=None, add_to_parent=True, **options):
        ...
        type = self.type
        return type(
            *type.prepare(dict(self.options, **options), tasks, args),
...
class chord(Signature):
    def __init__(
        self, header, body=None, task="celery.chord", args=(), kwargs={}, **options
    ):
        Signature.__init__(
            self,
            task,
            ...
            dict(
                ...
                body=maybe_signature(body, app=self._app),
            ),
            **options
        )
